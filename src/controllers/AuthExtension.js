...
module.exports.request_fp_token = async function(req, res) {
	try {
		...
		const { success, status, message, plain, hashed } = await Token.generateToken(isUser.email, 'forgotPassword')
		if (!success) {
...
module.exports.request_ev_token = async function(req, res) {
	try {
		...
		const { success, status, message, plain, hashed } = await Token.generateToken(isUser.email, 'verifyEmail')
		if (!success) {
			return sendJsonResponse(res, status, success, message);
		}
...
module.exports.request_cp_token = async function(req, res) {
	try {
		...
		const { success, status, message, plain, hashed } = await Token.generateToken(isUser.email, 'changePassword')
		if (!success) {
			return sendJsonResponse(res, status, success, message);
		}
...
module.exports.request_ev_token = async function(req, res) {
	try {
		...
		const { success, status, message, plain, hashed } = await Token.generateToken(isUser.email, 'changeEmail')
		if (!success) {
			return sendJsonResponse(res, status, success, message);
		}
